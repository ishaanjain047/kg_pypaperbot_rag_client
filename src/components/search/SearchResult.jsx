...
import { ArrowRight, ChevronDown, ChevronUp, ExternalLink, Star, Dna, Activity, Database, Grid, Zap, Brain, FileText } from "lucide-react";
...
const Chip = ({ text, icon: Icon, colorClass }) => {
  return (
    <div className={`inline-flex items-center ${colorClass} rounded-full px-3 py-1 text-sm font-medium mr-2 mb-2`}>
...
const PhenotypeChip = ({ phenotype }) => {
  return (
    <Chip text={phenotype} icon={Activity} colorClass="bg-purple-50 text-purple-700" />
  );
};
...
const ProcessChip = ({ process }) => {
  return (
    <Chip text={process} icon={Brain} colorClass="bg-emerald-50 text-emerald-700" />
  );
};
...
const FunctionChip = ({ func }) => {
  return (
    <Chip text={func} icon={Zap} colorClass="bg-blue-50 text-blue-700" />
  );
};
...
const ItemGrid = ({ items, ChipComponent, title, icon: Icon, colorClasses, maxDisplay = 20 }) => {
  ...
  return (
    <div className={`mb-6 ${colorClasses.bg} p-3 rounded-lg`}>
      <h3 className={`${colorClasses.text} text-sm font-semibold flex items-center mb-2`}>
        {Icon && <Icon size={16} className="mr-2" />}
        {title}
      </h3>
      <p className={`text-xs ${colorClasses.textLight} mb-2`}>
        {items.length} {title.includes("Genes") ? "gene" : 
                        title.includes("Phenotypes") ? "phenotype" : 
                        title.includes("Processes") ? "biological process" : 
                        title.includes("Functions") ? "molecular function" : 
                        title.includes("Diseases") ? "disease" : ""} 
        {items.length === 1 ? "" : "s"} specific to this drug
      </p> {/* Closed <p> tag properly here */}

      <div className="flex flex-wrap">
        {displayItems.map((item, index) => (
          <ChipComponent key={index} {...{ [ChipComponent.propName || "item"]: item }} />
        ))}
      </div>
...
const ScoreBadge = ({ score }) => {
  ...
  return (
    <div className="flex items-center">
      <div className={`rounded-full px-3 py-1 font-medium text-sm ${colorClasses[color]}`}>
      ...
      <div className="flex ml-2">
        {[1, 2, 3, 4, 5].map((star) => (
          <Star 
            ...
            stroke={star <= Math.round(numericScore/20) ? "#FFD700" : "#D1D5DB"}
            className={`${star <= Math.round(numericScore/20) ? "text-yellow-500" : "text-gray-300"}`}
          />
        ))}
      </div>
    </div>
...
const EvidenceBadge = ({ type }) => {
  const badges = {
    ...
    disease_similarity: { bg: "bg-blue-100", text: "text-blue-800", icon: Activity },
    disease_hierarchy: { bg: "bg-cyan-100", text: "text-cyan-800", icon: Activity },
    phenotype_gene: { bg: "bg-teal-100", text: "text-teal-800", icon: Dna },
    molecular_function: { bg: "bg-green-100", text: "text-green-800", icon: Zap },
    biological_process: { bg: "bg-emerald-100", text: "text-emerald-800", icon: Brain }
  };
  ...
  const badge = badges[type] || { bg: "bg-gray-100", text: "text-gray-800", icon: Activity };
  const Icon = badge.icon;
  const displayName = type.replace(/_/g, ' ');

  return (
    <span className={`${badge.bg} ${badge.text} px-3 py-1 rounded-full font-medium text-xs flex items-center`}>
  ...
  if (!result || typeof result !== 'object') {
    return (
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-5">
        <h3 className="text-lg font-semibold text-gray-800 mb-2">Invalid Result</h3>
    ...
    return (
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden transition-all hover:shadow-md">
        <div className="p-5">
          ...
          {/* Evidence types */}
          {result.evidence_types && Array.isArray(result.evidence_types) && result.evidence_types.length > 0 && (
            <div className="mb-4">
          ...
          {relatedDataCount > 0 && (
            ...
            >
              ...
              {showRelatedData ? <ChevronUp size={16} className="ml-1" /> : <ChevronDown size={16} className="ml-1" />}
            </button>
          )}
