...
export const searchDrugs = async (query, filters = {}) => {
  try {
    ...
    if (!data.top_candidates || !Array.isArray(data.top_candidates)) {
      console.error("Invalid response format: top_candidates missing or not an array");
      return [];
    }
    ...
    const formattedResults = data.top_candidates.map(candidate => {
      ...
      const score = typeof candidate.combined_score === 'number' 
        ? +(candidate.combined_score.toFixed(4)) 
        : (typeof candidate.score === 'number' ? +(candidate.score.toFixed(4)) : 0);
      
      // Format method scores for display
      const methodScoresFormatted = formatMethodScores(candidate.method_scores);
      
      // Format weights for display
      const weightsFormatted = formatWeights(candidate.weights_used || data.weights_used);
      
      // Format evidence types for display
      const evidenceTypesFormatted = formatEvidenceTypes(candidate.evidence_types);
      ...
      return {
        ...
        evidence_types: Array.isArray(candidate.evidence_types) ? candidate.evidence_types : [],
        
        // Handle both array and dictionary formats for related data
        related_genes: typeof data.related_genes === 'object' ? 
          (Array.isArray(data.related_genes) ? data.related_genes : 
           (candidate.drug && data.related_genes[candidate.drug] ? data.related_genes[candidate.drug] : [])) : [],
        
        related_phenotypes: typeof data.related_phenotypes === 'object' ? 
          (Array.isArray(data.related_phenotypes) ? data.related_phenotypes : 
           (candidate.drug && data.related_phenotypes[candidate.drug] ? data.related_phenotypes[candidate.drug] : [])) : [],
        
        related_molecular_functions: typeof data.related_molecular_functions === 'object' ? 
          (Array.isArray(data.related_molecular_functions) ? data.related_molecular_functions : 
           (candidate.drug && data.related_molecular_functions[candidate.drug] ? data.related_molecular_functions[candidate.drug] : [])) : [],
        
        related_biological_processes: typeof data.related_biological_processes === 'object' ? 
          (Array.isArray(data.related_biological_processes) ? data.related_biological_processes : 
           (candidate.drug && data.related_biological_processes[candidate.drug] ? data.related_biological_processes[candidate.drug] : [])) : [],
        
        related_diseases: typeof data.related_diseases === 'object' ? 
          (Array.isArray(data.related_diseases) ? data.related_diseases :
           (candidate.drug && data.related_diseases[candidate.drug] ? data.related_diseases[candidate.drug] : [])) : [],
        summary: candidate.summary || `${candidate.drug || "This drug"} is a repurposing candidate for ${data.disease || query} with a confidence score of ${score.toFixed(1)}.`,
        analysis: {
          drug_repurposing_recommendation: candidate.recommendation || 
            `${candidate.drug || "This drug"} is recommended as a potential repurposing candidate for ${data.disease || query} with a confidence score of ${score.toFixed(1)} out of 100.
...
This score represents the combined evidence from multiple computational analyses including gene associations, phenotype matching, and pathway analysis.`,
          ...
          biological_evidence: candidate.biological_evidence || 
...
${Array.isArray(candidate.evidence_types) && candidate.evidence_types.length > 0 ? 
  `The drug shows promising biological connections to ${data.disease || query} through multiple mechanisms and pathways.` : 
  ""}

Analysis Method Scores:
${methodScoresFormatted}`,
          
          mechanism_analysis: candidate.mechanism || 
            `The repurposing potential of ${candidate.drug || "this drug"} for ${data.disease || query} is supported by the following analysis methods:
...
${score >= 75 ? 
  ...
  score >= 50 ? 
    ...
    "This drug shows some mechanistic potential that warrants further investigation."}`,
          ...
          scientific_support: candidate.references || 
